...
export function guardResolved(result, options = {}) {
    ...
    if (result.kind === "skill") {
        ...
        try {
            ...
            if (!stat.isFile())
                return refusal("INVALID_REF", "resolved SKILL.md is not a regular file");
            if (stat.size > MAX_SKILL_BYTES) {
                return refusal("TOO_LARGE", `SKILL.md is ${stat.size} bytes; maximum is ${MAX_SKILL_BYTES} bytes`);
            }
        }
        catch {
