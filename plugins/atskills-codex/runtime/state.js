...
import { addTriggerLine, diskPath, expandLocalTriggers, frontmatter, hasTriggerLine, installLineFor, isCloud, nearestSource, normalizeId, parseTriggers, removeTriggerLine, resolveSkill as upstreamResolveSkill, saveSkillToProject as upstreamSaveSkillToProject, safeJoin, skillsRoot, writeFileAtomic, SOURCE_FILE, DEFAULT_CACHE_DIR, } from "./atskills.js";
import { existsSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, renameSync, rmSync, statSync, } from "node:fs";
import { dirname, join, sep } from "node:path";
import { tmpdir } from "node:os";
import { isSafeWorkspacePath, MAX_SKILL_BYTES, resolveSafely, } from "./security.js";
...
function resultCode(message, fallback = "NETWORK") {
    ...
    if (/invalid|empty skill path|path escapes/i.test(text))
        return "INVALID_REF";
    if (/too large|over the \d+|maximum is|exceed/i.test(text))
        return "TOO_LARGE";
...
function snapshotStats(dir) {
    ...
    const visit = (current) => {
        for (const entry of readdirSync(current, { withFileTypes: true })) {
            ...
            if (!entry.isFile())
                continue;
            if (entry.name === SOURCE_FILE || entry.name === ".visibility")
                continue;
            totals.files += 1;
...
function cacheRevision(cacheDir, id) {
    ...
    try {
        const metadata = JSON.parse(readFileSync(join(cacheDir, ".meta", `${hash}.json`), "utf8"));
        if (metadata &&
...
function sourceForSave(id, options) {
    if (!isCloud(id))
        return resolveSkill(id, false, options).then((resolved) => ({ resolved, temp: null }));
...
export async function saveSkill(rawId, options = {}) {
    ...
    if (!resolved?.success) {
        if (preflightTemp)
            rmSync(preflightTemp, { recursive: true, force: true });
        return failure(resolved?.code ?? resultCode(resolved?.error, "NOT_FOUND"), resolved?.error ?? `Unable to resolve '${id}'`);
    }
    ...
    if (limitError)
        return failure("TOO_LARGE", limitError, { bytes: stats.bytes, files: stats.files });
    ...
    if (indexWarning)
        warning = warning ? `${warning}; ${indexWarning}` : indexWarning;
...
export function removeSkill(rawId, options = {}) {
    ...
    if (!existsSync(dest))
        return failure("NOT_FOUND", `saved skill '${id}' does not exist`);
...
export function rebuildWorkspaceIndex(workingDir = process.cwd()) {
    ...
    const index = {
        ...
        skills,
        provenance: skills
            .filter((skill) => Boolean(skill.saved && skill.provenance))
...
export function readWorkspaceIndex(workingDir = process.cwd()) {
    try {
        const parsed = JSON.parse(readFileSync(workspacePaths(workingDir).index, "utf8"));
...
export function readProvenance(rawId, workingDir = process.cwd()) {
    ...
    if (!existsSync(dest))
        return null;
    if (!isSafeWorkspacePath(root, dest, true))
        return null;
    const stamp = nearestSource(dest, root);
    if (!stamp)
        return null;
