...
function absoluteSkillPath(value) {
    if (typeof value !== "string" || !value.endsWith("SKILL.md"))
        return null;
...
function renderContext(blocks, warnings = []) {
    ...
    if (omitted > 0)
        output.push(omissionLine(omitted));
...
function revisionFor(result, provenance) {
    ...
    if (typeof revision === "string" && revision && revision !== "unknown")
        return revision;
...
function collectionBlocks(result) {
    ...
    if (!id || entries.length === 0)
        return { blocks: [], warning: `collection '${id || "(unknown)"}' is empty` };
...
export async function buildPromptContext(input, runtime) {
    ...
    if (references.length > MAX_REFERENCES) {
        warnings.push(`Ignored ${references.length - MAX_REFERENCES} additional @skills reference${references.length - MAX_REFERENCES === 1 ? "" : "s"}; the maximum is ${MAX_REFERENCES} per prompt.`);
    }
    const resolverOptions = optionsFor(input);
    let resolved;
    try {
        resolved = await runtime.resolveSkillReferences(selected.map((reference) => reference.raw).join(" "), resolverOptions);
    }
    catch (error) {
    ...
    for (let index = 0; index < selected.length; index += 1) {
        ...
        if (result?.kind === "skill") {
            ...
            const block = skillBlock(provenance
                ? { ...result, saved: true, revision: result.revision ?? provenance.revision }
                : result, provenance ? [provenance] : undefined);
            if (block)
                blocks.push(block);
            else
                warnings.push(`${text(reference.raw)}: resolver returned no absolute SKILL.md path`);
            continue;
        }
...
export function buildSessionContext(_input, state) {
...
if (invokedFile === realpathSync(hookFile))
    await main();
