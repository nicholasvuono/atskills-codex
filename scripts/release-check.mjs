...
expect(pkg?.scripts?.build === "node scripts/build.mjs", "build script changed unexpectedly");
expect(pkg?.scripts?.test === "node --test test/*.test.mjs", "test script changed unexpectedly");
expect(pkg?.scripts?.check === "node scripts/check.mjs", "check script must run scripts/check.mjs");
...
const pluginEntry = marketplace?.plugins?.find((entry) => entry?.name === "atskills-codex");
...
expect(promptHook?.command === 'node "${PLUGIN_ROOT}/hooks/atskills.mjs"', "prompt hook command is incorrect");
