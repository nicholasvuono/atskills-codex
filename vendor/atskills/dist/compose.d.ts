...
/** The two suffixes, per reference. Both default to false. */
export interface ReferenceFlags {
    save?: boolean;
    install?: boolean;
}
/** Resolves ONE reference. Whatever the host already uses. */
export type ResolveOne<T> = (id: string, save: boolean, install: boolean) => Promise<T>;
/**
 ...
 */
export declare function resolveMany<T>(ids: string[], flags: ReferenceFlags[], resolveOne: ResolveOne<T>, onError: (id: string, error: Error) => T): Promise<T[]>;
/**
 * `resolveMany` bound to this package's resolver — the convenience path for a
 * host that has no resolver of its own.
 */
export declare function resolveSkills(ids: string[], opts: SkillResolverOpts, flags?: ReferenceFlags[]): Promise<LoadResponse[]>;
