 ...
 */
export async function resolveMany<T>(
  ids: string[],
  flags: ReferenceFlags[],
  resolveOne: ResolveOne<T>,
  onError: (id: string, error: Error) => T,
): Promise<T[]> {
  ...
  await Promise.all(
    ids.map(async (id, i) => {
      ...
      if (!p) {
        p = resolveOne(id, false, false).catch((e) =>
...
export async function resolveSkills(
  ...
  return resolveMany<LoadResponse>(
    ids,
    flags,
    (id, save, install) => resolveSkill(id, save, opts, install),
    (_id, error) => ({ success: false, error: error.message }),
  );
}
